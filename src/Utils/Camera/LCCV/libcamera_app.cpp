...
#include <Utils/Camera/LCCV/libcamera_app.hpp>
#include <Utils/Camera/LCCV/libcamera_app_options.hpp>
...
void LibcameraApp::ConfigureStill(unsigned int flags) {
      ...
      if (!configuration_)
            throw std::runtime_error(
                "failed to generate still capture configuration");
      ...
      // if (have_raw_stream && !options_->rawfull)
      {
            ...
            configuration_->at(1).size.height =
                configuration_->at(0).size.height;
      }
      configuration_->at(1).bufferCount = configuration_->at(0).bufferCount;

      configureDenoise(options_->denoise == "auto" ? "cdn_hq"
                                                   : options_->denoise);
      setupCapture();
...
void LibcameraApp::ConfigureViewfinder() {
      ...
      if (!configuration_)
            throw std::runtime_error(
                "failed to generate viewfinder configuration");
      ...
      configureDenoise(options_->denoise == "auto" ? "cdn_off"
                                                   : options_->denoise);
      setupCapture();
...
void LibcameraApp::StopCamera() {
      {
            // We don't want QueueRequest to run asynchronously while we stop
            // the camera.
      ...
      if (camera_)
            camera_->requestCompleted.disconnect(
                this, &LibcameraApp::requestComplete);

      // An application might be holding a CompletedRequest, so queueRequest
      // will get called to delete it later, but we need to know not to try and
      // re-queue it.
...
void LibcameraApp::queueRequest(CompletedRequest *completed_request) {
      ...
      // re-starts the camera, after which we don't want to queue another
      // request now.
      {
...
libcamera::Stream *LibcameraApp::GetMainStream() const {
      for (auto &p : streams_) {
            if (p.first == "viewfinder" || p.first == "still" ||
                p.first == "video")
                  return p.second;
      }
...
void LibcameraApp::setupCapture() {
      ...
      // Next allocate all the buffers we need, mmap them and store them on a
      // free list.
      ...
      for (StreamConfiguration &config : *configuration_) {
            ...
            for (const std::unique_ptr<FrameBuffer> &buffer :
                 allocator_->buffers(stream)) {
                  // "Single plane" buffers appear as multi-plane here, but we
                  // can spot them because then planes all share the same fd. We
                  // accumulate them so as to mmap the buffer only once.
      ...
      while (true) {
            for (StreamConfiguration &config : *configuration_) {
                  ...
                  if (stream == configuration_->at(0).stream()) {
                        ...
                        throw std::runtime_error("concurrent streams need "
                                                 "matching numbers of buffers");
