...
void ColorCalibrationThread::updateYellowLine(Detection::Line line) {
    std::unique_lock<std::mutex> lock(m_mutex);
    yellowLine = line;
}

void ColorCalibrationThread::updateBlueLine(Detection::Line line) {
    std::unique_lock<std::mutex> lock(m_mutex);
    blueLine = line;
}

CalibData ColorCalibrationThread::getCalibData() {
    std::unique_lock<std::mutex> lock(m_mutex);
